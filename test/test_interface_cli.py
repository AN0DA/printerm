...
from printerm.interfaces.cli import app
...
def mock_printer() -> Generator[MagicMock]:
    with patch("printerm.interfaces.cli.ThermalPrinter") as mock_printer_class:
        yield mock_printer_class


@pytest.fixture
def mock_get_printer_ip() -> Generator[None]:
    with patch("printerm.interfaces.cli.get_printer_ip", return_value="192.168.1.100"):
        yield


def test_print_template_command(mock_printer: MagicMock, mock_get_printer_ip: None) -> None:
    with patch("printerm.interfaces.cli.TemplateManager") as mock_template_manager:
