...
class TestWebInterface:
    ...
    @patch("printerm.interfaces.web.template_service")
    def test_api_preview_template_success(self, mock_service: MagicMock, client: FlaskClient) -> None:
        """Test successful template preview API."""
        mock_service.get_template.return_value = {
            "name": "Test Template",
        ...
        mock_service.render_template.return_value = [
            ...
            {"text": "Content here", "styles": {"bold": True}},
        ]

        response = client.post(
            "/api/preview/test_template", json={"title": "Test Title"}, content_type="application/json"
        )

        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
        assert "Title: Test Title" in data["preview"]
        assert data["segments_count"] == 2

    @patch("printerm.interfaces.web.template_service")
    def test_api_preview_template_with_script(self, mock_service: MagicMock, client: FlaskClient) -> None:
        """Test template preview API with script-based template."""
        mock_service.get_template.return_value = {"name": "Script Template", "script": "test_script"}
        mock_service.has_script.return_value = True
        mock_service.generate_template_context.return_value = {"generated": "content"}
        mock_service.render_template.return_value = [{"text": "Generated: content\n", "styles": {}}]

        response = client.post("/api/preview/script_template", json={}, content_type="application/json")

        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
        assert "Generated: content" in data["preview"]

    @patch("printerm.interfaces.web.template_service")
    def test_api_preview_template_error(self, mock_service: MagicMock, client: FlaskClient) -> None:
        """Test template preview API error handling."""
        mock_service.get_template.side_effect = Exception("Template not found")

        response = client.post("/api/preview/invalid_template", json={}, content_type="application/json")
    ...
    @patch("printerm.interfaces.web.template_service")
    def test_api_preview_empty_content(self, mock_service: MagicMock, client: FlaskClient) -> None:
        """Test preview API with empty content."""
        mock_service.get_template.return_value = {"name": "Empty Template", "variables": []}
        mock_service.has_script.return_value = False
        mock_service.render_template.return_value = [{"text": "\n\n\n", "styles": {}}]

        response = client.post("/api/preview/empty_template", json={}, content_type="application/json")

        assert response.status_code == 200
        data = response.get_json()
        assert data["success"] is True
        # Should have fallback message for empty content
        assert "whitespace" in data["preview"] or data["preview"].strip() == ""
